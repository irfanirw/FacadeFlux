...
{
    /// <summary>
    /// Previews EttvModel surfaces in Rhino viewport with colors by construction type.
    /// </summary>
    public class DisplayEttvConstructionTypeComponent : GH_Component
    {
        private readonly List<(Mesh mesh, Color color)> _previewMeshes = new();

        public DisplayEttvConstructionTypeComponent()
            : base("Display Ettv Construction Type", "DEM-Types",
                   "Displays EttvSurface geometry colored by construction type (fenestration vs opaque).",
                   "BcaEttv", "Utilities")
        { }

        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            pManager.AddGenericParameter("EttvModel", "M", "Model containing EttvSurface objects", GH_ParamAccess.item);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            var surfaces = model.Surfaces?.Where(s => s != null).ToList() ?? new List<EttvSurface>();
            if (surfaces.Count == 0)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Model contains no EttvSurface data.");
                DA.SetDataList(0, null);
                DA.SetData(1, "Model contains no EttvSurface data.");
                return;
            }
            ...
            foreach (var surface in surfaces)
            {
                ...
                var color = surface.Construction is EttvFenestrationConstruction
                ...
                if (surface.Construction is EttvFenestrationConstruction)
                    fenCount++;
                else
                    opaqueCount++;
            }
        ...
        private static bool TryGetModel(IGH_DataAccess DA, out EttvModel model)
        {
            model = null;
            object raw = null;
            ...
            if (raw is IGH_Goo goo)
            {
                ...
                if (script is EttvModel scriptModel)
                {
                    model = scriptModel;
                    return true;
                }
        ...
        private static Mesh DuplicateMesh(EttvSurface surface)
        {
